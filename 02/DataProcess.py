...
def getDatasetFromFile(path):
    ...
    for dir in dirs:
        ...
        for i in range(150):
            ...
            imginfo = ImageInfo(img, label)
            imginfo.getImgFeature()
            imginfo.normalizeSIFT()
            imgTrainSet.append(imginfo)
            feature_set = np.append(feature_set, imginfo.descriptors, axis=0)
